...
flags.DEFINE_float(
    'stop_threshold', default=0.712, help=('Stop threshold for MLPerf.'))
...
def get_pretrain_dataset_fn(input_file_pattern, seq_length,
                            ...
                            is_training, use_synthetic, num_eval_samples):
...
def run_customized_training(
    strategy, optimizer_type, weight_decay_rate, beta_1, beta_2, epsilon,
    bert_config, max_seq_length, max_predictions_per_seq, model_dir,
    steps_per_epoch, steps_per_loop, epochs, initial_lr, warmup_steps,
    input_files, train_batch_size, do_eval, eval_batch_size, num_eval_samples,
    custom_callbacks, init_checkpoint, steps_between_eval,
    steps_before_eval_start, stop_threshold, explicit_allreduce,
    allreduce_bytes_per_pack, enable_checkpoint_and_summary, device_warmup):
  ...
  mlp_log.mlperf_print('start_warmup_step', 0)
  mlp_log.mlperf_print('opt_epsilon', epsilon)
  mlp_log.mlperf_print('eval_samples', num_eval_samples)
  mlp_log.mlperf_print('opt_lamb_learning_rate_decay_poly_power', 1.0)
  ...
  train_input_fn = get_pretrain_dataset_fn(
      input_file_pattern=input_files,
...
# Hparams wrapper should be REDACTED only, not open source.
def hparams_run_wrapper(hparams):
  """Convenience wrapper for running with hyperparameters."""
  print('HParams: ', hparams)

  if 'learning_rate' in hparams:
    print('Overwriting learning_rate with ', hparams.learning_rate)
    FLAGS.learning_rate = hparams.learning_rate
  if 'beta_1' in hparams:
    print('Overwriting beta_1 with ', hparams.beta_1)
    FLAGS.beta_1 = hparams.beta_1
  if 'beta_2' in hparams:
    print('Overwriting beta_2 with ', hparams.beta_2)
    FLAGS.beta_1 = hparams.beta_2
  if 'warmup_steps' in hparams:
    print('Overwriting warmup_steps with ', hparams.warmup_steps)
    FLAGS.warmup_steps = hparams.warmup_steps
  if 'num_steps_per_epoch' in hparams:
    print('Overwriting num_steps_per_epoch with ', hparams.num_steps_per_epoch)
    FLAGS.num_steps_per_epoch = hparams.num_steps_per_epoch
  if 'weight_decay_rate' in hparams:
    print('Overwriting weight_decay_rate with ', hparams.weight_decay_rate)
